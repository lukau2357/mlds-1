...
def dt_tki(boot_samples):
    """
    Builds a deicison tree for the given dataset, with uncertainty estimation.
    We will generate a 95% CI for the MC rate using the percentile bootstrap technique.
        bootstrap_samples - Number of bootstrap samples to take for uncertainty quantification.
    """
    ...
    # seed set to 1 for reproducibility
    rnd = random.Random(1)

    for b in range(boot_samples):
        boot_sample_indices = rnd.choices(list(range(X.shape[0])), k = X.shape[0])

        X_boot = X[boot_sample_indices, :].copy()
        y_boot = y[boot_sample_indices].copy()

        X_boot_train, y_boot_train = X_boot[:130, :], y_boot[:130]
        X_boot_test, y_boot_test = X_boot[130:, :], y_boot[130:]

        (train_mc, train_se), (test_mc, test_se) = hw_tree_full((X_boot_train, y_boot_train),  (X_boot_test, y_boot_test))

        boot_mc.append(test_mc)
    
    return boot_mc

def rf_tki(boot_samples):
    """
    Build a random forest for the given dataset, with uncertainity estimation.
    """
    ...
    for b in range(boot_samples):
        ...
        (train_mc, train_se), (test_mc, test_se) = hw_randomforests((X_boot_train, y_boot_train),  (X_boot_test, y_boot_test))
